  ...
  (function(){
    ...
    var chatOptions = {
      ...
      modules: {
        ...
        IgnoreUser: {
          ...
          loaded: false,
          load: function () {
            importScriptPage("Debug/Chat/IgnoreUser.js","scripts");
            this.loaded = true;
          }
        }
      }
    ...
    function updateCookie() {
      ...
      for (var l in chatOptions.look) {
        if ( chatOptions.look.hasOwnProperty( l ) ) {
    ...
    // add the footer
    if (!$('#chat-plugin-footer').length) {
      $('.Rail').prepend('<div id="chat-plugin-footer" />');
    }
    
    // add options button
    if (!$('#chat-options-button').length) {
      // Prevent multiple buttons from being appended
      $('#chat-plugin-footer').append('<div id="chat-options-button"><img height="16" width="16" class="sprite gear" src="data:image/gif;base64,R0lGODlhAQABAIABAAAAAP///yH5BAEAAAEALAAAAAABAAEAQAICTAEAOw%3D%3D" /><div class="chat-options">Options</div></div>')
      $('#chat-options-button').click(openOptions);
    }
